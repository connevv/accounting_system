...
void clientComunicationService(int client_connection_fd){
	...
	while(1) {
		if(read(client_connection_fd, &option, sizeof(int))<0)
		{printf("ERROR READING FROM CLIENT");
		};
		switch(option)
		{
		case 1: addNewExpense(client_connection_fd); break;
		case 2: deleteExpense(client_connection_fd); break;
		case 3: printExpenses(client_connection_fd);break;
		case 4: getBiggestExpense(client_connection_fd);break;
		case 5: getMonthlyExpense (client_connection_fd); break;
